...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pagestat: '当前显示第 {from} 条到 {to} 条 总共 {total} 条',
            procmsg: '数据加载中, 请稍等 ...',
			 query: '',
			 qtype: '',
			 nomsg: '暂无数据',
		...
		var g = {
			...
			changePage: function (ctype){ //change page
				...
				switch(ctype)
				{
					...
					case 'next': if (p.page<p.pages) p.newp =1+ parseInt(p.page); break;
						...
						function ()
							{
									...
									$(this).dblclick(function(){
									    if(typeof(griddblclick)=="function")
									    {
									        //griddblclick(this);
									        griddblclick(prnt);
									    }
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>快速查询 <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='查询' /> <input type='button' value='全部' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[value=查询]', g.sDiv).click(function() { g.doSearch(); });
				$('input[value=全部]', g.sDiv).click(function() { $('input[name=q]', g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
