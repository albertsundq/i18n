...
function MM_OpenTabLayers(){//v1.0
	...
	newTab.src = Url;
	newTab.height=500;
	newTab.width = $("div_Main").offsetWidth;
	TabPage();
	loadingRrl = url;
...
function ShowImageDialog(intWidth, intHeight, obj)
{
    top.Show1(obj);
//    var url = "/Manage/Modle/UploadFile/ListFiles.aspx?Num=" + Math.random();
//    window.showModalDialog(url, null,  'dialogWidth:' + intWidth + 'px;dialogHeight:'+ intHeight + 'px;help:no;status:no;');
}
//验证批量删除
 function checkdel(f)
    {
...
function setAddTime()
{
  ...
  var arrobj=document.getElementByAddTime();
  for(var i=0;i<arrobj.length;i++)
  {
    if(arrobj[i].type=="text")
        arrobj[i].value=getToday();
    
  }
}

  document.getElementByAddTime = function () {
 
   var elements = [];

   var allobj = document.all ? document.all : document.getElementsByTagName("*");
   for (var i=0; i<allobj.length;i++)
   {



      var Names = allobj[i].name;
       
      if (Names!= undefined &&allobj[i].type=="text" && Names.substring(0,3)=="txt"&&Names.substring(Names.length-8,Names.length)=="_AddTime")
      {
     
      elements[elements.length] = allobj[i];
      }
   }
   return elements;
}
